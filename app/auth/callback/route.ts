...
export async function GET(request: Request) {
  ...
  if (code) {
    ...
    if (error) {
      ...
      return NextResponse.redirect(new URL(`/login?error=callback_failed&detail=${encodeURIComponent(error.message)}`, url.origin))
    }
  ...
  if (tokenHash && type) {
    ...
    if (error) {
      ...
      return NextResponse.redirect(new URL(`/login?error=verification_failed&detail=${encodeURIComponent(error.message)}`, url.origin))
    }
    return NextResponse.redirect(new URL(next, url.origin))
  }
