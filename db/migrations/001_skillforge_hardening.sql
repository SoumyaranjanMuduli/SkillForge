...
--
-- FRESH PROJECT? Don't run this file. db/schema.sql already contains everything
-- 001-004 add (see the "Production additions (also available as db/migrations/...)"
-- comments inside schema.sql) -- running these on top of a freshly-applied schema.sql
-- will fail with "policy already exists" / "type value already exists" style errors.
-- This migrations/ folder exists only to bring an OLDER, already-live database
-- (one created before schema.sql was consolidated) up to the current schema, by
-- applying 001 -> 002 -> 003 -> 004 in order and never running schema.sql again.
