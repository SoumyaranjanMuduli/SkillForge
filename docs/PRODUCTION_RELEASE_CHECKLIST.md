...
1. Apply `db/migrations/001` through `009` in order to an existing database. A fresh install should use `db/schema.sql` alone instead — it's the consolidated result of all nine migrations, so don't run both.
