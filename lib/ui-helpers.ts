import { BarChart3, Code2, Database, FileSpreadsheet, Sigma, type LucideIcon } from 'lucide-react'
...
export function programVisual(name: string): { Icon: LucideIcon; bg: string; text: string; bar: string } {
  ...
  if (n.includes('numpy')) return { Icon: Sigma, bg: 'bg-violet-50', text: 'text-violet-600', bar: 'bg-violet-500' }
  if (n.includes('pandas')) return { Icon: BarChart3, bg: 'bg-sky-50', text: 'text-sky-600', bar: 'bg-sky-500' }
